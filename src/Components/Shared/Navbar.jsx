...
const Navbar = ({ likedMeals }) => {
  ...
  return (
    <header className="bg-base-100 text-base-content shadow-md fixed top-0 w-full z-50 transition-all">
      <nav className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between h-16">
        ...
        <div className="flex items-center gap-3">
          ...
          <img src={img} alt="Logo" className="h-8 w-8 md:w-12 md:h-14 rounded-full" />
          <Link
            to="/"
            className="text-md hidden md:flex md:text-2xl font-bold bg-gradient-to-r from-primary to-secondary text-transparent bg-clip-text"
          >
            HostelHub
          </Link>
        </div>
          ...
          {navLinks.map(({ to, label }) => (
            <li key={to}>
              <NavLink
                to={to}
                className={({ isActive }) =>
                  `btn btn-sm btn-ghost text-sm font-medium rounded-md transition-all ${
                    isActive
          ...
          {user && (
            <li>
              <button
                ...
                className="btn btn-sm btn-outline text-sm font-medium hover:bg-primary hover:text-white focus:outline focus:outline-primary"
              >
                Dashboard
              </button>
            </li>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="dropdown dropdown-end">
            ...
            <ul
              tabIndex={0}
              className="menu dropdown-content mt-3 p-2 shadow bg-base-100 rounded-box w-40 text-sm"
            >
              <li>
      ...
      {/* Mobile Dropdown */}
      {isOpen && (
        <div className="lg:hidden px-4 pb-4 pt-2 bg-base-100 shadow">
          <ul className="flex flex-col gap-1">
            ...
            {user && (
              <li>
                <button
                  onClick={handleDashboard}
                  className="btn btn-outline btn-sm w-full mt-1 hover:bg-primary hover:text-white"
                >
                  Dashboard
                </button>
              </li>
